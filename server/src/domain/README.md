...
- `version.ts`: the process-wide live event version counter, starting at 1 (`specs/04-live-sync.md` §5, D-017). The socket layer takes version 1 for the state at start-up, and a snapshot sent to one socket carries the current version without taking one (D-104).
