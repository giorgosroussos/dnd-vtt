...
describe('a rectangle over N × N squares', () => {
  it('gives size = its side ÷ N and the offset from its corner, within one square', () => {
    ...
    // Whole squares covering the map: 4000 / 70.4 = 56.8 and 3000 / 70.4 = 42.6 round up (review).
    expect(result).toMatchObject({ columns: 57, rows: 43 });
  });
...
describe('fine tuning and the starting point', () => {
  it('keeps the size as typed, wraps the offsets into one square and derives the extent from the size', () => {
    const result = fromFine(ORIGINAL, 70.4, -3.25, 12.5);
    expect(result).toMatchObject({ size: 70.4, offset_y: 12.5, columns: 57, rows: 43 });
    // -3.25 draws the same lines as 70.4 - 3.25; an offset of two million does too, wrapped (review).
    expect(result.offset_x).toBeCloseTo(67.15, 9);
    expect(fromFine(ORIGINAL, 50, 2_000_010, 0).offset_x).toBeCloseTo(10, 6);
    expect(extentFor({ width: 10, height: 10 }, 400)).toEqual({ columns: 1, rows: 1 });
    // A map an exact number of squares wide needs no extra one.
    expect(extentFor({ width: 1000, height: 720 }, 40)).toEqual({ columns: 25, rows: 18 });
  });
  ...
  it('starts from the finest drawable size when the stored one is too fine to draw (review)', () => {
    expect(initialCalibration({ ...GRID, size: 1e-12 }, ORIGINAL).size).toBe(4000 / 2000);
    expect(initialCalibration({ ...GRID, columns: 100_000 }, ORIGINAL).size).toBe(4000 / 2000);
  });
...
describe('the far-corner magnifier', () => {
  it('shows the bottom-right region two squares wide, magnified, with the lines that fall in it', () => {
    const view = cornerView({ size: 50, offset_x: 10, offset_y: 0 }, ORIGINAL);
    expect(view.crop).toEqual({ x: 3900, y: 2900, width: 100, height: 100 });
    expect(view.zoom).toBe(MAGNIFIER_PX / 100);
    // Lines at 10 + 50k: 3910, 3960 inside the region; rows at 2900, 2950, 3000.
    expect(view.xs).toEqual([10, 60].map((at) => at * view.zoom));
    expect(view.ys).toEqual([0, 50, 100].map((at) => at * view.zoom));
  });
  ...
  it('never magnifies less than 1 × nor reaches past a small map', () => {
    ...
    expect(cornerView({ size: 5, offset_x: 0, offset_y: 0 }, ORIGINAL).crop.width).toBe(40);
  ...
  it('draws no lines for a size too fine to step, instead of never finishing (review)', () => {
    for (const size of [1e-12, 5e-324, 0.001]) {
      const view = cornerView({ size, offset_x: 0, offset_y: 0 }, ORIGINAL);
      expect(view.xs, String(size)).toEqual([]);
      expect(view.ys, String(size)).toEqual([]);
    }
    expect(cornerView({ size: 0.5, offset_x: 0, offset_y: 0 }, ORIGINAL).xs).toHaveLength(81);
  });
});
...
describe('the draft the panel edits', () => {
  it('turns each method into a calibration and writes it back into the other methods’ fields', () => {
    ...
    expect(draft.fields).toMatchObject({ size: '30', columns: '134', rows: '100' });
  });
  ...
  it('steps fine-tuning values by the arrow keys without float noise, and keeps the last good calibration for a bad field', () => {
    ...
    // The field keeps what was stepped to; the calibration holds the same grid wrapped into one square.
    expect(draft.fields.offset_x).toBe('-1');
    expect(draft.calibration.offset_x).toBe(69.5);
