...
- `MapCanvas.tsx`: the canvas in `dm` mode (camera controls, faint overlay when hidden for players, D-026) or `player` mode (no controls, fitted, no overlay when hidden). The DM view uses it from PRP-02; the player view from LIV-03.
