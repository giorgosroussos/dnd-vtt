...
`make verify` runs the first seven gate commands; `make audit`, `make scan-secrets` and `make tripwire` run beside it. Caches (npm, Playwright browsers) are keyed on `package-lock.json`, and a failed `e2e` job keeps `e2e/test-results/` as an artifact for seven days. For a red pipeline to block a merge, `main` needs branch protection requiring every job above; that setting lives on GitHub, not in this repository.
